...
sensorywheelClass <- if (requireNamespace('jmvcore', quietly=TRUE)) R6::R6Class(
  ...
  private = list(
    ...
    .run = function() {
      ...
      nrows=dim(data0)[1]
      ncols=dim(data0)[2]
      if (is.null(self$options$levv) || ncols<1 || nrows<1){return()}
      ...
      if (ncols>0){
        ...
        basecolors <- switch(   
          color1,   
          "JamoviStylish"=colorRampPalette(c("#3e6da9","#6B9DE8","lightgray","#E6AC40","#ebbc66"))(npalette),
        ...
        image$setState(data0) # plot data
        self$results$wheeloptions$setState(wheelcolors) # color data
      }
    },
    .plot=function(image,...) {
      x0=self$data[self$options$levv]
      nrows=dim(x0)[1]
      ncols=dim(x0)[2]
      if (is.null(self$options$levv) || ncols<1 || nrows<1){return()}
      ...
      # Here problems with reticulate package (windows arch) !!
      plotData=ggsunburst::sunburst_data(data0, type = "lineage", sep=",",node_attributes =add_attribute) # ete3 python package required !!
      ...
      # wheel + colors
      ap=private$.sunburst(plotData, node_labels = TRUE, node_labels.min = 1, leaf_labels.size = leaf, node_labels.size = node, leaf_labels.color = labelcolor,node_labels.color = labelcolor,rects.fill.aes = "name") + ggplot2::scale_fill_manual(values = wheelcolors, guide = FALSE)
      #ap=ggsunburst::sunburst(plotData, node_labels = TRUE, node_labels.min = 1, leaf_labels.size = leaf, node_labels.size = node, leaf_labels.color = labelcolor,node_labels.color = labelcolor,rects.fill.aes = "name") + ggplot2::scale_fill_manual(values = wheelcolors, guide = FALSE)
      self$results$showheelplot$setContent(ap) # show plot
      TRUE
    },
    .sunburst=function(data,rects.fill="white",rects.fill.aes=0,rects.color="black",rects.size=.5,blank=T,leaf_labels=T,leaf_labels.size = 2,leaf_labels.color = "black",node_labels = F,node_labels.size = 2,node_labels.color = "black",node_labels.min = 90){
      p=ggplot2::ggplot(data$rects)
      if (blank){p=p+ggplot2::theme_void()}
      # arcs
      if (rects.fill.aes == 0){
        p=p+ggplot2::geom_rect(ggplot2::aes_string(xmin="xmin",xmax="xmax",ymin="ymin",ymax="ymax"),fill=rects.fill,size=rects.size,color=rects.color)
      }else{
        p=p+ggplot2::geom_rect(ggplot2::aes_string(xmin="xmin",xmax="xmax",ymin="ymin",ymax="ymax",fill=rects.fill.aes),size=rects.size,color=rects.color)
      }
      # Leaf labels
      if (leaf_labels){p=p + ggplot2::geom_text(data=data$leaf_labels,ggplot2::aes_string(x="x", y="y", label="label", angle="angle"),size=leaf_labels.size, color=leaf_labels.color, hjust=.5)}
      # node labels
      if (node_labels){p=p + ggplot2::geom_text(data=data$node_labels[data$node_labels$delta_angle >= node_labels.min, ],ggplot2::aes_string(x="x", y="y", label="label", angle="pangle", vjust="pvjust"),size=node_labels.size, color=node_labels.color, hjust=.5)}
      # wheel
      p+ggplot2::xlab("")+ggplot2::ylab("")+ ggplot2::coord_polar()+ggplot2::ylim(min(data$rects$ymin)-1,NA)
    }) # Close - List
) # Close - R6::R6Class
