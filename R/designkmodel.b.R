designkmodelClass <- if (requireNamespace('jmvcore', quietly=TRUE)) R6::R6Class(
    ...
    private = list(
        ...
        .run = function() {
            ...
            image3$setState(modelo)

            image4 <- self$results$halfplot
            image4$setState(modelo)
            ...
            npe=length(contour.lm(modelo_rsm,as.formula(baseformu),image=FALSE,plot.it=FALSE))
            self$results$colsrsm$setState(npe)
            
            image5 <- self$results$rsmplot
            image5$setState(modelo_rsm)
        ...
        .mainplot=function(image1,...) {
          ...
          ap=main_effects(plotdata,response=self$options$yield,n_columns=ncolumns,color_palette ='turbo')
        ...
        .interactionplot=function(image2,...) {
          ...
          ncolumns=switch(tvars,1,2,3,2,3,3,4,4,5,5)
          if (is.null(ncolumns)){ncolumns=5}
          interaction_effects(plotdata,response=self$options$yield, colors = c("#6B9DE8", "#E6AC40"),linetypes = c("solid", "dashed"),n_columns=ncolumns)
          TRUE
        },
        .paretoplot=function(image3,...) {
          ...
          modelo=image3$state
          alphaval=(self$options$alphaval)/100
          ap=pareto_plot(modelo,method =self$options$criteria,alpha=alphaval)
          self$results$showplot$setContent(ap)
          TRUE
        },
        .halfplot=function(image4,...){
          ...
          modelo=image4$state
          alphaval=(self$options$alphaval)/100
          ap=half_normal(modelo,method=self$options$criteria,alpha=alphaval,ref_line=TRUE,label_active=TRUE,margin_errors=TRUE)
          self$results$showplot$setContent(ap)
          TRUE
        },
        .rsmplot=function(image5,...) {
          ...
          par(mfcol = c(npe,2))
          contour.lm(modeloR,as.formula(baseformu),image=TRUE,img.col = Jamov,decode = TRUE,main=self$options$yield)
          persp.lm(modeloR,as.formula(baseformu),contours="colors",col=Jamov,theta=azimut,phi=latitude,decode = TRUE,zlab=self$options$yield, font.lab=2,col.lab=33)
          TRUE
        }) # Close - List
) # Close - R6::R6Class
